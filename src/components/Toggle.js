...
export default class Toggle extends PureComponent {
  ...
  handleTouchMove(event) {
    ...
    if (this.startX != null) {
      ...
      if (this.props.checked && currentX + 15 < this.startX) {
        this.setState({ checked: false });
        this.startX = currentX;
      } else if (!this.props.checked && currentX - 15 > this.startX) {
  ...
  handleTouchEnd(event) {
    ...
    if (this.startX != null) {
      if (this.previouslyChecked !== this.props.checked) {
        checkbox.click();
      }
  ...
  render() {
    ...
    const classes =
      'react-toggle' +
      (this.props.checked ? ' react-toggle--checked' : '') +
